...
async function fetchPixabayQuota(apiKey) {
  if (!apiKey) {
    return { ok: false, configured: false, message: "MISSING" };
  }
  ...
  url.searchParams.set("q", "quota-check");
  ...
  try {
    ...
    return { ok: false, configured: true, message: body.slice(0, 120) };
  }

  if (data.error) {
    return { ok: false, configured: true, message: data.error };
  }
  ...
  return {
    ...
    note: "官方限額約 100 次/分鐘（API 不回傳剩餘次數）",
  };
}
...
async function collectQuotaStatus(ctx) {
  ...
  const [pexels, unsplash, pixabay, shutterstock] = await Promise.all([
    fetchPexelsQuota(pexelsKey),
    fetchUnsplashQuota(unsplashKey),
    fetchPixabayQuota(pixabayKey),
    fetchShutterstockQuota(sstkToken),
  ]);

  return { pexels, unsplash, pixabay, shutterstock };
}
...
function printQuotaStatus(ctx, quotas) {
  ...
  const { pexels, unsplash, pixabay, shutterstock } = quotas;
  ...
  console.log("── Pixabay（免費）──");
  if (!pixabay.configured) {
    console.log("  狀態   : 未設定 PIXABAY_API_KEY");
  } else if (!pixabay.ok) {
    console.log(`  狀態   : 錯誤 HTTP ${pixabay.status || ""}`);
    console.log(`  訊息   : ${pixabay.message}`);
  } else {
    console.log(`  搜圖   : ${pixabay.note}`);
    console.log("  下載   : 免費（需標註 Pixabay / 作者）");
    console.log("  中文   : 支援 lang=zh（關鍵字含中文時自動啟用）");
  }
  console.log("");
...
async function runQuota(_argv, ctx) {
  ...
  const anyError =
    ...
    (quotas.pixabay.configured && !quotas.pixabay.ok) ||
