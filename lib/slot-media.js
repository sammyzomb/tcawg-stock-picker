...
const DEFAULT_VIDEO_PROVIDER_PRIORITY = ["shutterstock", "pexels"];
...
function providerSupportsMediaType(provider, mediaType) {
  if (mediaType === "video") {
    return provider === "shutterstock" || provider === "pexels";
  }
  return provider === "shutterstock" || provider === "unsplash" || provider === "pexels" || provider === "commons";
}
